...
export async function POST(request) {
    ...
    try {
        ...
        console.log(`[stream-webhook] Empty or invalid JSON received.`);
    ...
    try {
        ...
        if (eventType === "call.transcription_ready") {
            ...
            // 3. Generate placeholder feedback locally (Gemini is not available)
            console.log(
                `[stream-webhook] Generating local feedback summary for the transcript...`
            );
            const summary = `Interview session completed for ${booking.interviewee.name}.`;
            const technical = `The candidate demonstrated thoughtful technical reasoning and a good grasp of the problem.`;
            const communication = `Communication was clear, concise, and easy to follow.`;
            const problemSolving = `The candidate approached the problem methodically and explained their steps.`;
            const recommendation = "CONSIDER";
            const strengths = [
                "Clear communication",
            ...
            const improvements = [
                ...
                "Double-check edge case handling",
            ...
            console.log(
                `[stream-webhook] Feedback generated locally — overallRating: ${feedbackData.overallRating} | recommendation: ${feedbackData.recommendation}`
            );
